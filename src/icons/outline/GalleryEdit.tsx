import * as React from "react";
import type { SVGProps } from "react";
const SvgGalleryEdit = (props: SVGProps<SVGSVGElement>) => (
  <svg
    {...props}
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
  >
    <path
      fill="currentColor"
      d="M9 10.75c-1.52 0-2.75-1.23-2.75-2.75S7.48 5.25 9 5.25 11.75 6.48 11.75 8 10.52 10.75 9 10.75m0-4a1.25 1.25 0 1 0 0 2.5 1.25 1.25 0 0 0 0-2.5"
    />
    <path
      fill="currentColor"
      d="M15 22.75H9c-5.43 0-7.75-2.32-7.75-7.75V9c0-5.43 2.32-7.75 7.75-7.75h4c.41 0 .75.34.75.75s-.34.75-.75.75H9C4.39 2.75 2.75 4.39 2.75 9v6c0 4.61 1.64 6.25 6.25 6.25h6c4.61 0 6.25-1.64 6.25-6.25v-5c0-.41.34-.75.75-.75s.75.34.75.75v5c0 5.43-2.32 7.75-7.75 7.75"
    />
    <path
      fill="currentColor"
      d="M15.67 9.75c-.39 0-.75-.14-1.01-.41-.31-.31-.46-.77-.39-1.24l.2-1.39c.05-.35.26-.78.52-1.03l3.63-3.63c1.45-1.45 2.71-.62 3.33 0 .59.59.86 1.21.8 1.85-.05.51-.31 1-.8 1.48l-3.63 3.63c-.25.25-.68.46-1.03.52l-1.39.2c-.08.02-.16.02-.23.02m4-6.63-3.63 3.63c-.03.03-.08.14-.09.19l-.19 1.3 1.31-.18c.04-.01.15-.06.19-.1l3.63-3.63c.22-.22.35-.42.36-.57.02-.21-.19-.47-.36-.64-.52-.52-.76-.46-1.22 0"
    />
    <path
      fill="currentColor"
      d="M20.9 6.13c-.07 0-.14-.01-.2-.03a4.08 4.08 0 0 1-2.79-2.79c-.11-.4.12-.81.52-.92s.81.12.92.52c.24.84.91 1.52 1.76 1.76.4.11.63.53.52.92a.79.79 0 0 1-.73.54M2.67 19.7a.751.751 0 0 1-.42-1.37l4.93-3.31c1.08-.72 2.57-.64 3.55.19l.33.29c.5.43 1.35.43 1.84 0l4.16-3.57c1.06-.91 2.73-.91 3.8 0l1.63 1.4c.31.27.35.74.08 1.06-.27.31-.74.35-1.06.08l-1.63-1.4c-.5-.43-1.35-.43-1.84 0l-4.16 3.57c-1.06.91-2.73.91-3.8 0l-.33-.29c-.46-.39-1.22-.43-1.73-.08L3.1 19.58c-.14.08-.29.12-.43.12"
    />
  </svg>
);
export default SvgGalleryEdit;
